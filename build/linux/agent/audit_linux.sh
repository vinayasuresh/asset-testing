...
#  SAN INFO             #
#========================
if [ -z $(echo "$skip_sections" | grep "san,") ]; then
	if [ "$san_audit" = "y" ]; then
...
last_os_update=$(grep "upgrade" -n1 /var/log/apt/history.log  2>/dev/null | head -n1 | xargs | cut -f2-3 -d" ")
...
# USB SECTION                    #
##################################
if [ -z $(echo "$skip_sections" | grep "usb,") ]; then
	if [ "$debugging" -gt "0" ]; then
		echo "USB Info"
	fi
	IFS="$NEWLINEIFS"
...
# POLICY SECTION                 #
##################################
if [ -z $(echo "$skip_sections" | grep "policy,") ]; then
	if [ "$debugging" -gt "0" ]; then
		echo "Policy Info"
	fi
	IFS="$NEWLINEIFS"
...
# BIOS SECTION                   #
##################################
if [ -z $(echo "$skip_sections" | grep "bios,") ]; then
	if [ "$debugging" -gt "0" ]; then
		echo "BIOS Info"
	fi
...
# PROCESSOR SECTION              #
##################################
if [ -z $(echo "$skip_sections" | grep "processor,") ]; then
	if [ "$debugging" -gt "0" ]; then
		echo "Processor Info"
	fi
...
# MEMORY SECTION                 #
##################################
if [ -z $(echo "$skip_sections" | grep "memory,") ]; then
	if [ "$debugging" -gt "0" ]; then
		echo "Memory Info"
	fi
...
# MOTHERBOARD SECTION            #
##################################
if [ -z $(echo "$skip_sections" | grep "motherboard,") ]; then
	if [ "$debugging" -gt "0" ]; then
...
##################################
if [ -z $(echo "$skip_sections" | grep "optical,") ]; then
	if [ "$debugging" -gt "0" ]; then
...
# VIDEO CARDS SECTION            #
##################################
if [ -z $(echo "$skip_sections" | grep "video,") ]; then
	if [ "$debugging" -gt "0" ]; then
...
# SOUND CARDS SECTION            #
##################################
if [ -z $(echo "$skip_sections" | grep "sound,") ]; then
	if [ "$debugging" -gt "0" ]; then
...
# SHARES SECTION                 #
##################################
if [ -z $(echo "$skip_sections" | grep "share,") ]; then
	if [ "$debugging" -gt "0" ]; then
		echo "Shares Info"
	fi
	echo "	<share>" >> "$xml_file"
...
##################################
if [ -z $(echo "$skip_sections" | grep "network,") ]; then
	if [ "$debugging" -gt "0" ]; then
...
# ADDRESSES SECTION              #
##################################
if [ -z $(echo "$skip_sections" | grep "ip,") ]; then
	IFS="$NEWLINEIFS"
	echo "	<ip>" >> "$xml_file"
...
# DISK SECTION                   #
##################################
if [ -z $(echo "$skip_sections" | grep "disk,") ]; then
	if [ "$debugging" -gt "0" ]; then
		echo "Hard Disk Info"
	fi
...
if [ -z $(echo "$skip_sections" | grep "vm,") ]; then
	if [ "$debugging" -gt "0" ]; then
...
# NFS MOUNTS SECTION             #
##################################
if [ -z $(echo "$skip_sections" | grep "partition,") ]; then
...
# LOG SECTION                    #
##################################
if [ -z $(echo "$skip_sections" | grep "log,") ]; then
	if [ "$debugging" -gt "0" ]; then
	        echo "Log Info"
	fi
...
##################################
if [ -z $(echo "$skip_sections" | grep "variable,") ]; then
	IFS='
'
	if [ "$debugging" -gt "0" ]; then
...
# SWAP SECTION                   #
##################################
if [ -z $(echo "$skip_sections" | grep "swap,") ]; then
	if [ "$debugging" -gt "0" ]; then
		echo "Swap Info"
	fi
	IFS="$NEWLINEIFS"
...
# USER SECTION                   #
##################################
if [ -z $(echo "$skip_sections" | grep "user,") ]; then
	if [ "$debugging" -gt "0" ]; then
		echo "User Info"
	fi
	echo "	<user>" >> "$xml_file"
...
# GROUP SECTION                  #
##################################
if [ -z $(echo "$skip_sections" | grep "group,") ]; then
	IFS=$ORIGIFS
	if [ "$debugging" -gt "0" ]; then
		echo "Group Info"
	fi
...
# SERVICE SECTION                                      #
########################################################
if [ -z $(echo "$skip_sections" | grep "service,") ]; then
	if [ "$debugging" -gt "0" ]; then
		echo "Service Info"
	fi
...
# SERVER SECTION                                       #
########################################################
if [ -z $(echo "$skip_sections" | grep "server,") ]; then
	if [ "$debugging" -gt "0" ]; then
		echo "Server Info"
	fi
	...
	if [ -n "$test" ]; then
		...
		if [ -n $(which netstat) ]; then
			if [ -n "$rev_exists" ]; then
				...
				port=$(netstat -tulpn 2>/dev/null | grep apache | awk '{ print $4 }' | cut -d: -f2 | head -n1)
			fi
			ip=$(netstat -tulpn 2>/dev/null | grep apache | awk '{ print $4 }' | cut -d: -f1 | head -n1)
		fi
		if [ -z "$port" ] || [ -z "$ip" ]; then
			if [ -n $(which ss) ]; then
				port=$(ss -tulpn 2>/dev/null | grep apache | awk '{print $5}' | cut -d: -f2 | head -n1)
				ip=$(ss -tulpn 2>/dev/null | grep apache | awk '{ print $5 }' | cut -d: -f1 | head -n1)
			fi
		fi
		if [ "$ip" = "*" ]; then
			ip=""
		...
		if [ -n "$config_file" ]; then
			certificates=$(grep -r -h -i SSLCertificateFile "$config_file"/* 2>/dev/null | sed -e 's/^[ \t]*//' | grep -v ^# | sed 's/SSLCertificateFile//' | sed -e 's/^[ \t]*//' | sort | uniq)
	...
	if [ -n "$test" ]; then
		...
		if [ -n $(which netstat) ]; then
			if [ -n "$rev_exists" ]; then
				...
				port=$(netstat -tulpn 2>/dev/null | grep httpd | awk '{ print $4 }' | cut -d: -f2 | head -n1)
			fi
			ip=$(netstat -tulpn 2>/dev/null | grep httpd | awk '{ print $4 }' | cut -d: -f1 | head -n1)
		fi
		if [ -z "$port" ] || [ -z "$ip" ]; then
			if [ -n $(which ss) ]; then
				port=$(ss -tulpn 2>/dev/null | grep httpd | awk '{print $5}' | cut -d: -f2 | head -n1)
				ip=$(ss -tulpn 2>/dev/null | grep httpd | awk '{ print $5 }' | cut -d: -f1 | head -n1)
			fi
		fi
		if [ "$ip" = "*" ]; then
			ip=""
	...
	if [ -n "$test" ]; then
		...
		if [ -z "$status" ]; then
			...
			if [ -z "$status" ]; then
				status=$(systemctl status mysql 2>/dev/null | grep Active | awk '{ print $2 }')
			fi
		fi
		port=""
		ip=""
		if [ -n $(which netstat) ]; then
			if [ -n "$rev_exists" ]; then
				...
				if [ -z "$port" ]; then
					port=$(netstat -tulpn 2>/dev/null | grep maria | awk '{ print $4 }' | rev | cut -d: -f1 | rev | head -n1)
				fi
			else
				port=$(netstat -tulpn 2>/dev/null | grep mysql | awk '{ print $4 }' | cut -d: -f2 | head -n1)
				if [ -z "$port" ]; then
					port=$(netstat -tulpn 2>/dev/null | grep maria | awk '{ print $4 }' | cut -d: -f2 | head -n1)
				fi
			fi
			ip=$(netstat -tulpn 2>/dev/null | grep mysql | awk '{ print $4 }' | cut -d: -f1 | head -n1)
			if [ -z "$ip" ]; then
				ip=$(netstat -tulpn 2>/dev/null | grep maria | awk '{ print $4 }' | cut -d: -f1 | head -n1)
			fi
		fi
		if [ -z "$port" ] || [ -z "$ip" ]; then
			if [ -n $(which ss) ]; then
				port=$(ss -tulpn 2>/dev/null | grep mysql | awk '{print $5}' | cut -d: -f2 | head -n1)
				if [ -z "$port" ]; then
					port=$(ss -tulpn 2>/dev/null | grep maria | awk '{print $5}' | cut -d: -f2 | head -n1)
				fi
				ip=$(ss -tulpn 2>/dev/null | grep mysql | awk '{ print $5 }' | cut -d: -f1 | head -n1)
				if [ -z "$ip" ]; then
					ip=$(ss -tulpn 2>/dev/null | grep maria | awk '{ print $5 }' | cut -d: -f1 | head -n1)
				fi
			fi
		fi
		if [ "$ip" = "*" ]; then
	...
	if [ -n "$test" ]; then
		...
		if [ -n $(which netstat) ]; then
			if [ -n "$rev_exists" ]; then
				...
				port=$(netstat -tulpn 2>/dev/null | grep postgres | awk '{ print $4 }' | cut -d: -f2 | head -n1)
			fi
			ip=$(netstat -tulpn 2>/dev/null | grep postgres | awk '{ print $4 }' | cut -d: -f1 | head -n1)
		fi
		if [ -z "$port" ] || [ -z "$ip" ]; then
			if [ -n $(which ss) ]; then
				port=$(ss -tulpn 2>/dev/null | grep postgres | awk '{print $5}' | cut -d: -f2 | head -n1)
				ip=$(ss -tulpn 2>/dev/null | grep postgres | awk '{ print $5 }' | cut -d: -f1 | head -n1)
			fi
		fi
		if [ "$ip" = "*" ]; then
			ip=""
	...
	if [ -n "$test" ]; then
		...
		if [ -n $(which netstat) ]; then
			if [ -n "$rev_exists" ]; then
				...
				port=$(netstat -tulpn 2>/dev/null | grep nginx | awk '{ print $4 }' | cut -d: -f2 | head -n1)
			fi
			ip=$(netstat -tulpn 2>/dev/null | grep nginx | awk '{ print $4 }' | cut -d: -f1 | head -n1)
		fi
		if [ -z "$port" ] || [ -z "$ip" ]; then
			if [ -n $(which ss) ]; then
				port=$(ss -tulpn 2>/dev/null | grep nginx | awk '{print $5}' | cut -d: -f2 | head -n1)
				ip=$(ss -tulpn 2>/dev/null | grep nginx | awk '{ print $5 }' | cut -d: -f1 | head -n1)
			fi
		fi
		if [ "$ip" = "*" ]; then
			ip=""
	...
	if [ -n "$test" ]; then
		...
		if [ -n $(which netstat) ]; then
			if [ -n "$rev_exists" ]; then
				...
				port=$(netstat -tulpn 2>/dev/null | grep mongod | awk '{ print $4 }' | cut -d: -f2 | head -n1)
			fi
			ip=$(netstat -tulpn 2>/dev/null | grep mongod | awk '{ print $4 }' | cut -d: -f1 | head -n1)
		fi
		if [ -z "$port" ] || [ -z "$ip" ]; then
			if [ -n $(which ss) ]; then
				port=$(ss -tulpn 2>/dev/null | grep mongod | awk '{print $5}' | cut -d: -f2 | head -n1)
				ip=$(ss -tulpn 2>/dev/null | grep mongod | awk '{ print $5 }' | cut -d: -f1 | head -n1)
			fi
		fi
		if [ "$ip" = "*" ]; then
			ip=""
	...
	if [ -n "$datadir" ]; then
		...
		for i in $(find "$datadir" -type d 2>/dev/null | grep -o '[^/]*$'); do
	...
	IFS="$NEWLINEIFS"
	datadir=$(grep -IR data_directory /etc/postgresql/ 2>/dev/null | cut -d\' -f2)
	if [ -n "$datadir" ]; then
		if [ "$debugging" -gt "0" ]; then
	...
	if [ -z "$datadir" ]; then
		...
		if [ -n "$datadir" ]; then
			if [ "$debugging" -gt "0" ]; then
				echo "	postgresql using process list"
			fi
		fi
	fi

	if [ -n "$datadir" ]; then
		for j in $(echo "$datadir"); do
			if [ -d "$j" ]; then
				for i in $(find "$j/base/" -type d 2>/dev/null | grep -o '[^/]*$'); do
					size=$(ls -lk "$j/base/$i" | awk '{ total += $5 }; END { print total/1024/1024 }')
					oid2name=$(which oid2name 2>/dev/null)
					if [ -z "$oid2name" ]; then
						# try to find oid2name in path (for ubuntu)
						oid2name=$(find /usr/lib/postgresql/ -name oid2name 2>/dev/null | head -n1)
					fi
					myname=$i
					if [ -x "$oid2name" ]; then
						if [ $(whoami) = "root" ]; then
					...
					description=""
					if [ "$myname" = "postgres" ]; then
						description="default"
					fi
					if [ "$myname" = "template0" ]; then
						description="default"
					fi
					if [ "$myname" = "template1" ]; then
						description="default"
					fi
					{
					echo "		<item>"
					...
					echo "			<path>$(escape_xml "$j/base")/$(escape_xml "$i")</path>"
					echo "			<size>$(escape_xml "$size")</size>"
					echo "		</item>"
					} >> "$xml_file"
				done
...
##################################
if [ -z $(echo "$skip_sections" | grep "certificate,") ]; then
	if [ "$debugging" -gt "0" ]; then
...
########################################################
if [ -z $(echo "$skip_sections" | grep "netstat,") ]; then
	IFS="
";
	if [ "$debugging" -gt "0" ]; then
		echo "Netstat Info"
	...
	if [ -n $(which ss) ]; then
		netstatdump=$(ss -lntup 2>/dev/null | grep -v "Local Address" | awk '{print $1 "\t0\t0\t" $5 "\t0\tLISTEN\t" $7}')
	fi
...
# CUSTOM FILES                                         #
########################################################
if [ -z $(echo "$skip_sections" | grep "file,") ]; then
	if [ "$debugging" -gt "0" ]; then
...
# EXECUTABLES                                          #
########################################################
if [ -z $(echo "$skip_sections" | grep "executable,") ]; then
	if [ "$debugging" -gt "0" ]; then
...
# ============================================
# ITAM SOFTWARE COLLECTION
# Build JSON array of installed software for ITAM enrollment
# ============================================
ITAM_SOFTWARE_JSON=""
ITAM_SOFTWARE_COUNT=0
MAX_SOFTWARE_ITEMS=500  # Limit to avoid huge payloads

# Helper function to add software to JSON array
add_itam_software() {
	local name="$1"
	local version="$2"
	local publisher="$3"
	...
	[ "$ITAM_SOFTWARE_COUNT" -ge "$MAX_SOFTWARE_ITEMS" ] && return

	# Escape JSON special characters
	name=$(echo "$name" | sed 's/\\/\\\\/g; s/"/\\"/g; s/	/ /g' | tr -d '\n\r')
	version=$(echo "$version" | sed 's/\\/\\\\/g; s/"/\\"/g; s/	/ /g' | tr -d '\n\r')
	publisher=$(echo "$publisher" | sed 's/\\/\\\\/g; s/"/\\"/g; s/	/ /g' | tr -d '\n\r')

	# Add comma if not first item
	if [ -n "$ITAM_SOFTWARE_JSON" ]; then
		ITAM_SOFTWARE_JSON="$ITAM_SOFTWARE_JSON,"
	fi

	ITAM_SOFTWARE_JSON="$ITAM_SOFTWARE_JSON{\"name\":\"$name\",\"version\":\"$version\",\"publisher\":\"$publisher\"}"
	ITAM_SOFTWARE_COUNT=$((ITAM_SOFTWARE_COUNT + 1))
}

# Collect software based on distro (same logic as XML section)
if [ -z "$(echo "$skip_sections" | grep "software,")" ]; then
	if [ "$debugging" -gt 1 ]; then
		echo "Collecting software for ITAM enrollment..."
	fi

	# Add OS as first software item
	add_itam_software "$system_os_name" "$system_os_version" ""

	case $system_os_family in
		'Ubuntu' | 'Debian' | 'LinuxMint' | 'Raspbian' )
			while IFS=$'\t' read -r pkg ver; do
				add_itam_software "$pkg" "$ver" ""
			done < <(dpkg-query --show --showformat='${Package}\t${Version}\n' 2>/dev/null | head -n $MAX_SOFTWARE_ITEMS)
			;;
		'CentOS' | 'RedHat' | 'Fedora' | 'Suse' | 'Amazon' | 'Mariner' | 'AlmaLinux' )
			while IFS=$'\t' read -r pkg ver; do
				add_itam_software "$pkg" "$ver" ""
			...
			;;
		'Alpine Linux' )
			for package in $(apk info 2>/dev/null | head -n $MAX_SOFTWARE_ITEMS); do
				version=$(apk info "$package" 2>/dev/null | grep description | cut -d" " -f1 | cut -d "-" -f2-)
				add_itam_software "$package" "$version" ""
			done
			;;
		'Arch' )
			while read -r pkg ver; do
				add_itam_software "$pkg" "$ver" ""
			done < <(pacman -Q 2>/dev/null | head -n $MAX_SOFTWARE_ITEMS)
			;;
		* )
			# Try dpkg first, then rpm as fallback for unknown distros
	...
	if [ "$debugging" -gt 1 ]; then
		echo "Collected $ITAM_SOFTWARE_COUNT software items for ITAM enrollment"
	fi
fi
...
if [ -z "$ENROLLMENT_TOKEN" ]; then
	...
	# Include software array if collected
	if [ -n "$ITAM_SOFTWARE_JSON" ]; then
		ITAM_JSON=$(cat <<EOF
{
